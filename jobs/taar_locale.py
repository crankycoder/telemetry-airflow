...
def get_client_addons(spark, start_date):
    ...
    parquetFile = spark.read.parquet(s3_url)
...
def store_json_to_s3(json_data, base_filename, date, prefix, bucket):
    ...
    current_filename = "{}.json".format(base_filename)
    archive_fname = "{}{}.json".format(base_filename, date)

    _write_json_to_s3(json_data, archive_fname, prefix, bucket)
    _write_json_to_s3(json_data, current_filename, prefix, bucket)


def _write_json_to_s3(json_data, s3_dest_file_name, s3_prefix, bucket):

    s3 = boto3.resource("s3")
    keypath = s3_prefix + s3_dest_file_name
    logging.info("Writing to : s3://{}/{}".format(bucket, keypath))

    obj = s3.Object(bucket, keypath)
